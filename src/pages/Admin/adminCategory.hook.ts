...
export const useAdminCategoryes = (
  ...
  const [extendedSelectCategory, setExtendedSelectCategory] = useState<ICategoryForm>({} as ICategoryForm)
...
// }
  ...
  const useDeleteCategory = useCallback( async (category: ICategory, adminId: string) => {   
    setIsLoading(true)
    const config = {
      method: 'DELETE',
      url: `${basicUrl.urlAdminCategory}/delete/${category._id}/${adminId}`
    } as AxiosRequestConfig
  ...
  const useGetFieldTranslation = useCallback(async (selectCategory: ICategory, signal?: AbortSignal) => { 
    setIsLoading(true)    
    const config = {
      method: 'GET',
    ...
    axiosIC<ICategoryForm>(config)
      .then((response) => {
        console.log('Расширенная категория получена ', response.data)
        const resServer = response.data
        if (resServer) setExtendedSelectCategory(resServer)        
        setIsLoading(false)                     
      })   
      .catch((error: AxiosError) => {
        if (axios.isAxiosError(error)) {
  ...
  return {useGetCategoryList, useGetCategoryForNavBar, useGetCategoryListCount, useGetSearch, useGetCategoryListPagination, useAddNewCategory, useGetFieldTranslation, extendedSelectCategory, setExtendedSelectCategory, useEditCategory, useDeleteCategory, error, clearError}
} 