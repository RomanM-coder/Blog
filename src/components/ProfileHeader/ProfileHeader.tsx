...
export const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  const handleAvatarUpload = async (
    ...
    if (!file.type.startsWith('image/')) {
      myToast(t('profilePage.profileHeader.fileIsImage'), basicColor.orange)
      return
    }

    if (file.size > 2 * 1024 * 1024) {
      myToast(t('profilePage.profileHeader.fileSize'), basicColor.orange)
      return
    }
        ...
        <div className={styles.avatarContainer}>
          ...
          <div className={styles.avatarOverlay}>
            <label htmlFor="avatar-upload" className={styles.avatarUploadLabel}>
              {isUploading
                ? t('profilePage.profileHeader.loading')
                : t('profilePage.profileHeader.changeAvatar')}
            </label>
            <input
              id="avatar-upload"
              type="file"
