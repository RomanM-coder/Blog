...
instance.interceptors.request.use(
  (
    ...
    if (token) {
      ...
      // if (role) {
      //   config.headers['X-User-Role'] = role // ✅ Добавляем роль в отдельный header
      // }
    }
  ...
  (error) => {
    if (error.response) {
      ...
      if (status === 401) {
        ...
        // Просто делаем редирект и очищаем localStorage
        if (typeof window !== 'undefined') {
          localStorage.removeItem('userData')
          // Проверяем, что мы не уже на странице auth
